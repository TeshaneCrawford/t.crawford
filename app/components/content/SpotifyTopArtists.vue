...
const { data: topArtists, status } = useFetch<Artist[]>('/api/spotify/top-artists', {
  default: () => [],
})
...
<template>
  <section class="space-y-4" aria-labelledby="spotify-top-artists-heading">
    <div class="space-y-1">
      ...
      <p v-if="!isLoading && topArtists?.length" class="text-sm text-gray-12">
    ...
    >
      ...
      <template v-else>
        <div v-for="n in 6" :key="n" class="aspect-square animate-pulse rounded-lg bg-gray-4" />
      </template>
    </div>
  </section>
</template>
