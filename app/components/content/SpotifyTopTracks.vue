...
const { data: topTracks, status } = useFetch<Track[]>('/api/spotify/top-tracks', {
  default: () => [],
})
...
<template>
  <section aria-labelledby="top-tracks-heading" class="space-y-4">
    <div class="space-y-1">
      ...
      <p v-if="!isLoading && topTracks?.length" class="text-sm text-gray-11">
      ...
      <div v-if="isLoading" class="h-5 w-3/4 animate-pulse rounded bg-gray-4" />
    </div>
    <ul v-if="!isLoading" class="space-y-3" role="list">
      <li
    ...
    <ul v-else class="space-y-3" role="list">
      <li v-for="n in 5" :key="n" class="flex items-center gap-4 rounded-lg bg-gray-3 p-3">
        <div class="h-12 w-12 animate-pulse rounded-md bg-gray-4" />
        <div class="min-w-0 flex-1 space-y-2">
          <div class="h-4 w-2/3 animate-pulse rounded bg-gray-4" />
          <div class="h-3 w-1/2 animate-pulse rounded bg-gray-4" />
        </div>
        <div class="h-5 w-5 animate-pulse rounded-full bg-gray-4" />
      </li>
    </ul>
  </section>
</template>
