...
export async function useSearchResults(search: MaybeRefOrGetter<string>, options: { lazy?: boolean, server?: boolean } = {}): Promise<ComputedRef<SearchDisplay>> {
  ...
  return computed(() => {
    return searchResults.value.reduce((acc, item) => {
      const websiteValue = website.value as { search: { groups: { path: string }[] } };

      const pathname = window.location.pathname; // Add this line to declare the 'pathname' variable

      const group = websiteValue.search.groups.find(group => pathname.startsWith(group.path)) ?? {
        name: 'Pages',
        path: '',
      }
